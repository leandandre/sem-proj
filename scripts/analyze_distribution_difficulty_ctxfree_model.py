"""
This script scores the validation and the test nights individually (for the context-free model) to see what nights are hard to classify and if vaidation is more difficutl than test set.
"""
import numpy as np
import json
import sys
...
def main():
    ...
    lr_head = 1e-3
    mode = "headband"
    experiment_name = "ctxfree_stage1_p1.0_ssl_finetuning" # change accordingly
    ...
    val_metrics_file = TARGET_DIR_METRICS / "ctxfree_val_set_per_night_metrics.json"
    test_metrics_file = TARGET_DIR_METRICS / "ctxfree_test_set_per_night_metrics.json"
    ...
    if val_metrics_file.exists() and test_metrics_file.exists():
        ...
        for night_id in val_nights:
            ...
            night_acc, night_mf1, night_perclass_f1 = evaluate_model(model, night_dl, device)
    ...
    # plt.show()
    plt.savefig(TARGET_DIR_PLOTS / "ctxfree_val_set_per_night_mf1_distribution.pdf", dpi=300, bbox_inches='tight')
    ...
    plt.tick_params(axis='both', labelsize=16)
    # plt.show()
    plt.savefig(TARGET_DIR_PLOTS / "ctxfree_test_set_per_night_mf1_distribution.pdf", dpi=300, bbox_inches='tight')
    ...
    plt.tick_params(axis='both', labelsize=16)
    # plt.show()
    plt.savefig(TARGET_DIR_PLOTS / "ctxfree_val_and_test_set_per_night_mf1_distributions.pdf", dpi=300, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(TARGET_DIR_PLOTS / "ctxfree_val_and_test_set_per_night_mf1_violin_and_kde.pdf", dpi=300, bbox_inches='tight')
    


    # I see in the histogram that some nights in test set have mf1 < 0.5
    nights_low_mf1 = []
    for night_id in test_nights:
        nested = test_nights_metrics[night_id]
        if nested['night_mf1'] < 0.5:
            nights_low_mf1.append((night_id, nested['night_mf1']))
    print("\nTest Nights with Macro F1 Score below 0.5:")
    for night_id, mf1 in nights_low_mf1:
        print(f"Night ID: {night_id}, Macro F1 Score: {mf1:.4f}")
    
    # which participents are these nights?
    test_pids = splits['test_pids']
