"""
The training script for fine-tuning (or training supervised from scratch) a context-sensitive GRU (SequenceGRUClassifier) on top of a pretrained SSL epoch encoder (SSLEpochTransformerConv1D_v2).
"""
import json
...
def make_sequence_dataloaders(
    ...
    train_ds = BoasSequenceDataset(
        ...
        mode=mode,
        seq_len=seq_length,
        stride=stride,          # stride < seq_len for overlapping sequences for increased training data
    ...
    val_ds = BoasSequenceDataset(
        ...
        mode=mode,
        seq_len=seq_length,
        stride=seq_length,  # non-overlapping sequences for validation (this is change to procedure before)
        transform_hb=None,
        preprocess_config=preprocess_config,
        use_cache=use_cache,
    )
...
def train_contextsensitive_classifier_new(
        num_epochs: int = 100,
        batch_size: int = 16,
        seq_len: int = 20,
        stride: int = 5,
    ...
    return best_macro_f1, fin_acc, fin_per_class_f1, best_epoch